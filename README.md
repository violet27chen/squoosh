...
> **Cloudflare variant**: identical API (same `/image/<opts>/<path>` and `?url=`), different engine (Cloudflare native image resizing). Deploy with the Cloudflare button above — it deploys as a **Worker with static assets** (functionally equivalent to Pages, same `*.pages.dev` URL). The button reads the repo config to pre-fill everything: build command & deploy command (from `package.json` scripts), output dir `build/` (from `wrangler.toml`'s `[assets] directory`), and `ALLOWED_URL_HOSTS` (from `wrangler.toml`'s `[vars]`). No manual setup needed. Full details in [README.cloudflare.md](./README.cloudflare.md).
