...
> **Cloudflare variant**: identical API (same `/image/<opts>/<path>` and `?url=`), different engine (Cloudflare native image resizing). Deploy with the Cloudflare button above — unlike the EdgeOne button it takes **no** build-dir URL params; instead it reads the repo's config to pre-fill everything: build command `npm run build` (from `package.json`), output dir `build/` (from `wrangler.toml`'s `pages_build_output_dir`), and `ALLOWED_URL_HOSTS` (from `wrangler.toml`'s `[vars]`). No manual setup needed. Full details in [README.cloudflare.md](./README.cloudflare.md).
