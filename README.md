...
> **Cloudflare variant**: identical API (same `/image/<opts>/<path>` and `?url=`), different engine (Cloudflare native image resizing). Deploy with the Cloudflare button above — it reads `wrangler.toml`, so `ALLOWED_URL_HOSTS` and the `build/` output dir are pre-filled automatically (no manual env setup). Full details in [README.cloudflare.md](./README.cloudflare.md).
