...
> **Cloudflare 版本**：API 完全一致（同样的 `/image/<选项>/<路径>` 与 `?url=`），仅底层换成 Cloudflare 原生图片缩放。用上方 Cloudflare 按钮即可一键部署——与 EdgeOne 按钮不同，它**没有** `build-command` / `output-directory` 这类 URL 参数；预填靠读取仓库配置：构建命令 `npm run build`（来自 `package.json`）、输出目录 `build/`（来自 `wrangler.toml` 的 `pages_build_output_dir`）、环境变量 `ALLOWED_URL_HOSTS`（来自 `wrangler.toml` 的 `[vars]`），全自动，无需手动配。详见 [README.cloudflare.md](./README.cloudflare.md)。
