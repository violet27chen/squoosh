...
> **Cloudflare 版本**：API 完全一致（同样的 `/image/<选项>/<路径>` 与 `?url=`），仅底层换成 Cloudflare 原生图片缩放。用上方 Cloudflare 按钮即可一键部署——它会读取 `wrangler.toml`，自动预填 `ALLOWED_URL_HOSTS` 与 `build/` 输出目录，无需手动配环境变量。详见 [README.cloudflare.md](./README.cloudflare.md)。
